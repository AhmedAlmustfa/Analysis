...
df1 <- df %>% inner_join(ito, by = "LuogoAccadimento")
#head(df1)
```

```{r}
highchart(type = "map") %>% 
  hc_title(text = "The number of injuries by province") %>% 
  hc_add_series_map(map = ita, df = df1, joinBy = "hasc", value = "n")
```


The map as well confirmed that, the provinces with the highest number of injuries are Mialn Rome and Turin.
